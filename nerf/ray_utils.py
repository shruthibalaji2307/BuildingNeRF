...
def get_pixels_from_image(image_size, camera):
    ...
    x = ((x - torch.min(x))/(torch.max(x) - torch.min(x)) - 0.5) * 2
    y = ((y - torch.min(y))/(torch.max(y) - torch.min(y)) - 0.5) * 2
...
def get_rays_from_pixels(xy_grid, image_size, camera):
    ...
    ndc_points = torch.cat(
        [
            ndc_points,
            torch.ones_like(ndc_points[..., -1:]).cuda()
        ],
        dim=-1
    )
    ...
    rays_o = rays_o.repeat(world_points.shape[0], 1)

    # TODO (1.3): Get normalized ray directions
    rays_d = world_points - rays_o
    norm = rays_d.norm(p=2, dim=1)
    norm = norm.unsqueeze(1).repeat(1,3)
    rays_d = rays_d/norm
